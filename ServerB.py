...
class Bob(Participant):
    ...
    def send(self, cA, q, g, gk):
        ...
        if self.bit == 0:
            ...
            cB = (
                pow(cA[0], r_, p),
                multiply_modulo_big(pow(g, r_, p), pow(cA[1], r_, p), p),
            )
        return cB
...
def calculate():
    ...
    rst_data["cB"] = bob_instance.send(
        public_data_from_Alice["cA"],
        public_data_from_Alice["q"],
        public_data_from_Alice["g"],
        public_data_from_Alice["gk"],
    )
    print(rst_data)
